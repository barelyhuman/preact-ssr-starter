...
export default async function (app) {
  ...
  let manifest = `${pagePaths.map((path, ind) => `import * as View${ind} from '.${path.replace('src', '')}'`).join(';\n')}`

  manifest += `;export const paths = {${pagePaths.map((path, ind) => `"${path}":View${ind}`).join(',\n')}}`
